...
class aleatorios:
    def __init__(self, datos):
        ...
        self.decimales=2 #Numero de decimales por redodndear, valor por omision
        try:
            opts, args = getopt.getopt(self.datos, "hv:V:n:d:", ["help", "vminima=","vmaxima=","cuantos=","decimales"])
        except getopt.error:
            print('Modo de ejecutar el código: ')
            print('simula6.py -h -v -v <valor_minimo> -V <valor_maximo>[-n cuantos] [-d decimales]')
            print(' ')
        ...
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                ...
                por default de construir 15 números pseudoaleatorios, asi mismo, la variable 
                <d> representa el numero de cifras decimales por redondear, de no señalarse,
                se tomaran dos cifras decmales'''
                                                 )
                                                 
                parser.add_argument(
                    '-v', '--vminima', help='Inicio del intervalo', type=float, required=True)
                parser.add_argument(
                    '-V', '--vmaxima', help='Fin del intervalo', type=float, required=True)
                parser.add_argument(
                    '-n', '--cuantos', help='Número de pseudoaleatorios a ser generados', type=int, required=False)
                args = parser.parse_args()
                parser.add_argument(
                    '-d', '--decimales', help='Número de cifras decimales por redondear', type=int, required=False)
                args = parser.parse_args()
